...
% Lista de casos a comparar (escalones del diente de sierra)
steps_list = [5, 25, 100];
...
for idx = 1:length(steps_list)
    ...
    xlabel('Tiempo [s]'); ylabel('Amplitud');
    title(sprintf('%d pasos: Rx (retardado)', n_steps));
    xlim([0 T+delay_time]);
    ...
    xlabel('Tiempo [s]'); ylabel('Amplitud');
    title(sprintf('%d pasos: salida del matched filter', n_steps));
    xlim([0 T+delay_time]);
end
